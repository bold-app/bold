...
class DeviceDetectorTest < ActiveSupport::TestCase
  ...
  test "should recognize mobile browsers" do
    MOBILE.each do |ua|
      assert_equal :mobile, Bold::DeviceDetector.new(ua, @lang).device_class
    end
  end

  test "should recognize bots by UA" do
    BOTS.each do |ua|
      assert_equal :bot, Bold::DeviceDetector.new(ua, @lang).device_class
    end
  end

  test 'should recognize as bot if no language' do
    MOBILE.each do |ua|
      assert_equal :bot, Bold::DeviceDetector.new(ua, '').device_class
    end
    DESKTOP.each do |ua|
      assert_equal :bot, Bold::DeviceDetector.new(ua, '').device_class
    end
  ...
  test "should recognize desktop browsers" do
    DESKTOP.each do |ua|
      assert_equal :desktop, Bold::DeviceDetector.new(ua, @lang).device_class
    end
  end

  test "benchmark should recognize bots by UA" do
    skip 'skipped benchmark'
    3.times do
      r = Benchmark.measure do
        1000.times do
          MOBILE.each do |ua|
            assert_equal :mobile, Bold::DeviceDetector.new(ua, @lang).device_class, ua
          end
          DESKTOP.each do |ua|
            assert_equal :desktop, Bold::DeviceDetector.new(ua, @lang).device_class, ua
          end
          BOTS.each do |ua|
            assert_equal :bot, Bold::DeviceDetector.new(ua, @lang).device_class, ua
          end
        end
      end
      puts r
